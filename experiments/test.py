...
def main(
	...
	for nbatches, (image, label) in enumerate(dataloader):
		...
		accuracy, recall, precision, dice = im_metrics
		...
		print(f"Scan {nbatches}:\n"
            f"accuracy = {accuracy}\nrecall = {recall}\n"
            f"precision = {precision}\ndice = {dice}\n")
		log_iteration_metrics(
			im_metrics, steps=nbatches, writer=writer, data="test"
		)
	...
	results_df.to_csv(
		os.path.join(cache.out_dir_test, "test_results.csv"), index=False
	)
	writer.close()
