...
def postprocess_segmentation(segm_np, n_classes=5, bg_idx=0, multiple_organ_indici = [3]):
    ...
    for class_idx in range(n_classes):
        ...
        initial_segmentation_dilated = ndimage.binary_dilation(initial_segmentation, structure=struct).astype(initial_segmentation.dtype)
        # print("after", initial_segmentation_dilated.shape)
        valid_mask, num = label(initial_segmentation_dilated, connectivity=2, return_num=True)
        props = regionprops(valid_mask)
        props = sorted(props, key=lambda x: x.area, reverse=True)

        if len(props) == 0:
            pass
        elif (len(props) == 1) or (class_idx not in multiple_organ_indici) or (props[0].area > 1.5*props[1].area):
